...
// ── NUTRITION SYNC — recommend products by workout type ───────────────────────
export const recommendByWorkout = async (req, res, next) => {
  try {
    const WORKOUT_TAG_MAP = {
      strength:    ["high-protein", "recovery", "creatine"],
      cardio:      ["stamina", "electrolytes", "energy"],
      yoga:        ["recovery", "flexibility", "low-calorie"],
      hiit:        ["energy", "high-protein", "electrolytes"],
      powerlifting:["creatine", "high-protein", "recovery"],
      general:     ["high-protein", "recovery"],
    };

    const workoutType = req.params.workoutType?.toLowerCase() || "general";
    const tags = WORKOUT_TAG_MAP[workoutType] || WORKOUT_TAG_MAP.general;

    const products = await Product.find({
      isAvailable: true,
      tags: { $in: tags },
    })
      .populate("vendor", "businessName city verificationStatus")
      .limit(3);

    // Only approved vendors
    const verified = products.filter(p => p.vendor?.verificationStatus === "approved");

    // Fallback: if no tagged products, return top selling supplements
    if (verified.length === 0) {
      const fallback = await Product.find({ isAvailable: true, category:"supplement" })
        .populate("vendor", "businessName city verificationStatus")
        .sort({ totalSold: -1 })
        .limit(3);
      return res.json({ products: fallback.filter(p => p.vendor?.verificationStatus === "approved"), workoutType });
    }

    res.json({ products: verified, workoutType });
  } catch (error) { next(error); }
};